...
class TaskRepeater {
    ...
    _calcRepeatedDates() {
        console.log("NEW CALC-REPEAT-DATES");
        if (this._cloneTask.repeat && this._cloneTask.repeat.repeat) {
            if (this._cloneTask.repeat.everyInterval == "day") {
                this._cloneTask = this._calcRepeatedDays(this._cloneTask.repeat.everyNumber);
            }
            if (this._cloneTask.repeat.everyInterval == "week") {
                this._cloneTask = this._calcRepeatedDays(7 * this._cloneTask.repeat.everyNumber);
            }
            if (this._cloneTask.repeat.everyInterval == "month") {
                this._cloneTask = this._calcRepeatedMonth(this._cloneTask.repeat.everyNumber);
            }
            if (this._cloneTask.repeat.everyInterval == "year") {
                this._cloneTask = this._calcRepeatedYear(this._cloneTask.repeat.everyNumber);
            }
        }
    };
    ...
    _calcRepeatedDays (dayDelta) {
        ...
        if (this._cloneTask.startDate && !this._cloneTask.dueDate) {
            ...
            this._cloneTask.startDate = groundDate;   // setDate can't work accross month. We must pull it up to grounddate, first.
        ...
        if (!this._cloneTask.startDate && this._cloneTask.dueDate) {
            ...
            this._cloneTask.dueDate = groundDate;   // setDate can't work accross month. We must pull it up to grounddate, first.
